...
hubs_order <- c("Northwest", "California", "Southwest", "Northern Plains", "Southern Plains", "Midwest", "Southeast", "Northeast")

hubs_map <- albersusa::usa_sf() %>% 
...
# Plot regions map
map <- ggplot(hubs_map, aes(fill = hub)) +
  geom_sf(color = NA) +
...
max(dm_perc_cat_hubs_raw$MapDate)
```
...
dm_perc_cat_hubs <-
  ...
  ## Northern Forest == Midwest + Northeast
  filter(Name != "Northern Forests\\n") %>%
  ## Carribean rather boring
...
# Plot stacked bars (varying backgrou)
bars <- ggplot(dm_perc_cat_hubs, aes(week, percentage)) +
  geom_rect(aes(
    xmin = .5, xmax = max_week + .5,
    ymin = -0.005, ymax = 1),
    fill = "#f4f4f9", color = NA, size = 0.4, show.legend = FALSE) + #9d9ca7, 99a4be, 8696bd
  geom_col(aes(fill = category, fill = after_scale(addmix(darken(fill, .05, space = "HLS"), "#d8005a", .15)), 
  ...
  scale_fill_viridis_d(option = "rocket", direction = -1, begin = .17, end = .97, 
                       name = "Category:", labels = c("Abnormally Dry", "Moderate Drought", 
...
```{r climatic-hubs-stacked-bars-fixed, fig.width=14.5, fig.height=11.8, cache=TRUE}
# Wrangle
dm_perc_cat_hubs <-
  dm_perc_cat_hubs_raw %>%
  ## Northern Forest == Midwest + Northeast
  ...
  group_by(year) %>%
  mutate(max_week = ifelse(year == 2021, max(week), 53)) %>% ## for fixed
  ungroup() %>% 
  filter(percentage > 0)
...
ggsave(here::here("pitch", "drought_bars_facet_fixed.pdf"), width = 14.5, height = 11.8, device = cairo_pdf)
ggsave(here::here("pitch", "drought_bars_facet_fixed.svg"), width = 14.5, height = 11.8)
```
